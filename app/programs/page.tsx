...
export default function ProgramsPage() {
 ...
 const filteredPrograms = selectedCategory ? allPrograms.filter((p) => p.category === selectedCategory) : allPrograms
 ...
 <h1 className="text-5xl font-bold bg-gradient-to-r from-[#77E5C8] to-[#A7CBDC] bg-clip-text text-transparent mb-6">Tất cả chương trình</h1>
 <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
 Tìm chương trình thiện nguyện phù hợp với bạn và tạo nên sự khác biệt
 </p>
 </div>
 ...
 <Card key={program.id} className="group overflow-hidden border-0 bg-white/80 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-3">
 ...
 <div className="p-6">
 <h3 className="text-lg font-bold text-foreground mb-3 group-hover:text-[#6085F0] transition-colors duration-200">{program.name}</h3>
 <p className="text-muted-foreground text-sm mb-4 line-clamp-2">{program.description}</p>
 <div className="space-y-3 text-sm text-muted-foreground mb-6">
 <div className="flex items-center gap-2">📍 {program.location}</div>
 ...
 <Button className="w-full bg-gradient-to-r from-[#77E5C8] to-[#6085F0] hover:opacity-90 shadow-lg hover:shadow-lg transition-all duration-300" asChild>

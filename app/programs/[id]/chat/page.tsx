...
export default function ProgramChatPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 msg.userId === user?.id ? "bg-[#6085F0] text-white" : "bg-muted text-foreground"
 } rounded-lg p-4`}
 >
 ...
 <p className={`text-xs ${msg.userId === user?.id ? "text-white/90" : "text-muted-foreground"}`}>
 ...
 />
 <Button type="submit" className="bg-[#6085F0] hover:opacity-90">

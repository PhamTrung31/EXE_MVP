...
export default function CompleteProgramPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
    ...
    return (
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <main className="flex-1 container mx-auto px-4 py-12">
          <p className="text-muted-foreground">Chỉ tổ chức mới có thể truy cập trang này</p>
        </main>
        <Footer />
      </div>
    )
  ...
  if (!program || program.organizationId !== user.id) {
    return (
      <div className="min-h-screen flex flex-col bg-background">
        <Header />
        <main className="flex-1 container mx-auto px-4 py-12">
          <p className="text-muted-foreground">Không tìm thấy chương trình hoặc bạn không có quyền truy cập</p>
        </main>
        <Footer />
      </div>
    )
  ...
  const handleCompleteProgram = async () => {
    if (!window.confirm("Bạn có chắc muốn đánh dấu chương trình hoàn thành? Hành động này sẽ cấp chứng chỉ cho tất cả tình nguyện viên.")) {
      return
    }
    ...
    try {
      ...
      const existingCertificates = JSON.parse(localStorage.getItem("customCertificates") || "[]")
      ...
      localStorage.setItem("customCertificates", JSON.stringify([...existingCertificates, ...newCertificates]))
        ...
        <div className="container mx-auto px-4">
          ...
          <Card className="p-8 border-[#77E5C8]">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h1 className="text-3xl font-bold text-foreground mb-2">Hoàn thành chương trình</h1>
                <p className="text-muted-foreground">{program.name}</p>
              </div>
            </div>

            <div className="mb-8">
              <h2 className="text-xl font-bold text-foreground mb-4">Danh sách tình nguyện viên</h2>
              <p className="text-muted-foreground mb-6">
                Nhập số giờ tình nguyện cho từng người. Chứng chỉ sẽ được tự động cấp khi hoàn thành.
              </p>

              {approvedVolunteers.length === 0 ? (
                <p className="text-muted-foreground">Chưa có tình nguyện viên nào được duyệt</p>
              ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-semibold text-foreground">{volunteer?.name || reg.volunteerName}</p>
                            <p className="text-sm text-muted-foreground">{volunteer?.email}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <Label htmlFor={`hours-${reg.volunteerId}`} className="text-sm text-muted-foreground">
                            Số giờ:
                          </Label>
                          <Input
                            ...
                            onChange={(e) => handleHoursChange(reg.volunteerId, e.target.value)}
                            className="w-24"
                          />
                        </div>
                      </div>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
              <p className="text-yellow-800 text-sm">
                ⚠️ <strong>Lưu ý:</strong> Sau khi hoàn thành, chương trình sẽ chuyển sang trạng thái "Đã hoàn thành" và
                không thể chỉnh sửa.
              </p>
            </div>
            ...
            >
              {isLoading ? "Đang xử lý..." : `Hoàn thành & Cấp ${approvedVolunteers.length} chứng chỉ`}
            </Button>
          </Card>
        </div>
      </main>

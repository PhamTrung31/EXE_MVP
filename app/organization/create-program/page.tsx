...
export default function CreateProgramPage() {
 ...
 id: `prog-${Date.now()}`,
 ...formData,
 schedule: formData.schedule ? formData.schedule.split(",").map(s => s.trim()) : [],
 ...
 const existingPrograms = JSON.parse(localStorage.getItem("customPrograms") || "[]")
 localStorage.setItem("customPrograms", JSON.stringify([...existingPrograms, newProgram]))
 ...
 <h1 className="text-3xl font-bold text-foreground mb-8">Tạo chương trình mới</h1>
 ...
 className="rounded"
 />
 <Label htmlFor="isInternational">Chương trình quốc tế</Label>
 </div>

 {formData.isInternational && (
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-blue-50 rounded-lg">
 <div className="space-y-2">
 <Label htmlFor="domesticDescription">Mô tả tiếng Việt</Label>
 <Textarea
 id="domesticDescription"
 value={formData.domesticDescription}
 onChange={(e) => handleInputChange("domesticDescription", e.target.value)}
 rows={3}
 placeholder="Mô tả bằng tiếng Việt"
 />
 </div>
 <div className="space-y-2">
 <Label htmlFor="internationalDescription">Mô tả tiếng Anh</Label>
 <Textarea
 id="internationalDescription"
 value={formData.internationalDescription}
 onChange={(e) => handleInputChange("internationalDescription", e.target.value)}
 rows={3}

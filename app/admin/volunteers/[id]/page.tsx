...
export default function VolunteerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const volunteer = mockAccounts.find((acc) => acc.id === id && acc.role === "volunteer")
  ...
  if (!volunteer) {
    return (
      <div className="min-h-screen flex flex-col bg-background">
        <Header />
        <main className="flex-1 container mx-auto px-4 py-12">
          <p className="text-muted-foreground">Không tìm thấy tình nguyện viên</p>
        </main>
        <Footer />
      </div>
    )
  ...
  const totalHours = certificates.reduce((sum, cert) => sum + cert.hoursContributed, 0)
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              <Card className="p-6 border-0 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white shadow-lg">
                <div className="text-center">
                  ...
                  <p className="text-sm text-emerald-100">Chương trình đã tham gia</p>
                </div>
              </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  {registrations.length > 0 ? (
                    registrations.map((reg) => {
                      ...
                      return (
                        <div key={reg.id} className="p-4 border border-border rounded-lg hover:bg-emerald-50 transition">
                          ...
                          <p className="text-sm text-muted-foreground mb-2">{program.organizationName}</p>
              ...
              {/* Certificates */}
              <Card className="p-8 border-0 bg-white/80 backdrop-blur-sm shadow-xl">
                <h2 className="text-2xl font-bold text-foreground mb-6">Chứng chỉ</h2>
                <div className="space-y-4">
                  {certificates.length > 0 ? (
                    certificates.map((cert) => (
                      <div key={cert.id} className="p-4 border border-emerald-200 rounded-lg bg-emerald-50">
                        <div className="flex items-start justify-between mb-2">

...
export default function OrganizationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const organization = mockAccounts.find((acc) => acc.id === id && acc.role === "organization")
  ...
  const totalVolunteers = programs.reduce((sum, p) => sum + p.volunteersJoined, 0)
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-blue-50 via-white to-emerald-50">
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 py-8">
          <Button variant="ghost" asChild className="mb-6 hover:bg-emerald-50">
          ...
          <div className="max-w-6xl mx-auto">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-4 text-muted-foreground mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>Tham gia từ {organization.joinedDate || "2024"}</span>
                    </div>
                  </div>
                  {organization.bio && (
                    <p className="text-muted-foreground mb-3">{organization.bio}</p>
                  )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              ...
              <Card className="p-6 border-0 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white shadow-lg">
                <div className="text-center">
                  <CheckCircle className="w-8 h-8 mx-auto mb-2" />
                  <p className="text-2xl font-bold">{activePrograms.length}</p>
                  <p className="text-sm text-emerald-100">Đang hoạt động</p>
                </div>
              </Card>
            ...
            <Card className="p-8 border-0 bg-white/80 backdrop-blur-sm shadow-xl">
              <h2 className="text-2xl font-bold text-foreground mb-6">Chương trình</h2>
              <div className="space-y-4">
                {programs.length > 0 ? (
                  programs.map((program) => {
                    ...
                    return (
                      <div key={program.id} className="p-6 border border-border rounded-lg hover:bg-blue-50 transition">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <span>{program.volunteersJoined}/{program.volunteersNeeded} tình nguyện viên</span>
                              </div>
                            </div>
                          </div>
                          <span
                  ...
                  <p className="text-muted-foreground text-center py-8">Chưa có chương trình nào</p>
                )}
              </div>
            </Card>
          </div>
      ...
      {selectedProgram && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            ...
            >
              {/* Modal Header */}
              <div className="sticky top-0 bg-gradient-to-r from-emerald-600 to-emerald-700 p-6 flex items-center justify-between border-b border-emerald-500 rounded-t-2xl">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                  ...
                  <p className="text-emerald-100">{selectedProgram.organizationName}</p>
                </div>
                <button
                  onClick={() => setSelectedProgram(null)}
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-muted-foreground">
                    Danh mục: <span className="font-medium text-foreground">{selectedProgram.category}</span>
                  </span>
                </div>

                {/* Description */}
                <div>
                  <h3 className="text-lg font-semibold text-foreground mb-2">Mô tả chương trình</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedProgram.domesticDescription && (
                      <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                        <h4 className="font-semibold text-emerald-700 mb-2">Mô tả trong nước</h4>
                        <p className="text-sm text-muted-foreground">{selectedProgram.domesticDescription}</p>
                      </div>
                    )}
                    {selectedProgram.internationalDescription && (
                      <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                        <h4 className="font-semibold text-blue-700 mb-2">Mô tả quốc tế</h4>
                        <p className="text-sm text-muted-foreground">{selectedProgram.internationalDescription}</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div>
                      <p className="font-semibold text-foreground">Tình nguyện viên</p>
                      <p className="text-sm text-muted-foreground">
                        {selectedProgram.volunteersJoined}/{selectedProgram.volunteersNeeded} người
                      </p>
                  ...
                  {selectedProgram.duration && (
                    <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                      <Calendar className="w-5 h-5 text-emerald-600 mt-0.5" />
                      <div>
                        <p className="font-semibold text-foreground">Thời lượng</p>
                ...
                {(selectedProgram.schedule || selectedProgram.timeSlots) && (
                  <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                    <h3 className="text-lg font-semibold text-foreground mb-3">Lịch trình hoạt động</h3>
                    {selectedProgram.schedule && selectedProgram.schedule.length > 0 && (
                      <div className="mb-3">
                        <p className="text-sm font-semibold text-foreground mb-1">Ngày hoạt động:</p>
                        <p className="text-sm text-muted-foreground">{selectedProgram.schedule.join(", ")}</p>
                      </div>
                    )}
                    {selectedProgram.timeSlots && (
                      <div>
                        <p className="text-sm font-semibold text-foreground mb-1">Khung giờ:</p>
                        <p className="text-sm text-muted-foreground">{selectedProgram.timeSlots}</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {(() => {
                  ...
                  const approvedCount = programRegistrations.filter((r) => r.status === "approved").length
                  const pendingCount = programRegistrations.filter((r) => r.status === "pending").length
                  const rejectedCount = programRegistrations.filter((r) => r.status === "rejected").length

                  return (
                    <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                      <h3 className="text-lg font-semibold text-foreground mb-3">Thống kê đăng ký</h3>
                ...
                <div className="flex gap-3 pt-4 border-t border-border">
                  ...
                  >
                    <Link href={`/programs/${selectedProgram.id}`} target="_blank">
                      Xem trang đầy đủ
                    </Link>
                  </Button>
                </div>
